...
runCommand "prefix-with-gma3" { } ''
  ...
  # Install grandMA3
  ${lib.getExe wine} start /wait /unix "${sources.installer}" /S

  # Seed terminal config
  mkdir -p $WINEPREFIX/drive_c/ProgramData/MALightingTechnology/gma3_2.3.2/terminalapp/config
  echo -n -e '\x02\x00\x00\x00\x7f\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00' \
